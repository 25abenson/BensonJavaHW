...
public class PartC {
    public static void main(String[] args) throws Exception {
        ...
        int num = in.nextInt(); //changed from in.next to in.nextInt because variable must be an integer and therefore input has to be an integer

        // calculate the cost before discount.
        double cost = num * COST_OF_COOKIE;

        // Use conditional statements to determine what discount is.
        double discount = 0; //had to set discount value outside of the loop so it is initialized as a value
        //must organize loop from most restrictive to least 
        if (num > 24) {
            discount = 0.15;
        } else if (num > 12) {
            discount = 0.05;
        ...
        System.out.printf("For %d cookies, " , num); 
